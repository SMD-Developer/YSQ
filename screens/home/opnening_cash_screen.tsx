...
const TodaysCashScreen: React.FC<TodaysCashProps> = ({navigation, route}) => {
  ...
  const renderItem = ({item}: {item: OpenCashItem}) => {
    var formattedDate = format(new Date(item.created_at), 'dd MMM yyyy HH:mm');
    return (
      <View style={styles.itemContainer}>
        <Text style={styles.storeName}>
          {Const.languageData?.Cash_ID ?? 'Cash ID'}: {item.unique_id}
        </Text>
        <View style={styles.detailsContainer}>
          ...
          <View style={styles.dataRow}>
            <Text style={styles.detailText}>
              {Const.languageData?.Warehouse ?? 'Warehouse'}:{' '}
              <Text style={styles.detailValue}>
                {item.sales_man?.salesman[0]?.warehouse[0]?.name ?? ''}
              </Text>
            </Text>
          </View>
        </View>
  ...
  return (
    <View style={styles.container}>
      <MainAppBar
        title={`${
          isAll
            ? Const.languageData?.Cash_History
            : isOpeningCash
            ? Const.languageData?.Opening_Cash ?? 'Opening'
            : Const.languageData?.Closing_Cash ?? 'Closing'
        }`}
        isPrimary={false}
      />
        ...
        {isAll ? (
          <FlatList
            ...
            data={[
              ...
              {
                id: Const.languageData?.Opening ?? 'Open',
                name: Const.languageData?.Opening ?? 'Open',
              },
              {
                id: Const.languageData?.Closing ?? 'Close',
                name: Const.languageData?.Closing ?? 'Close',
              },
            ]}
            horizontal
      ...
      {loading ? (
        <Loader />
      ) : (filteredCashData?.data ?? []).length === 0 ? (
...
export default TodaysCashScreen;
